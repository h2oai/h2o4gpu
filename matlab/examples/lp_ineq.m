...
%%LP_INEQ Test POGS on an inequality constrained LP.
...
results.max_violation = abs(min(min(b - A * x_pogs), 0)) / norm(x_pogs);
results.avg_violation = mean(abs(min(b - A * x_pogs, 0)));
