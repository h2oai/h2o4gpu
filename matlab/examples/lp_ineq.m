...
%%LP_INEQ Test H2OAIGLM on an inequality constrained LP.
...
results.max_violation = abs(min(min(b - A * x_h2oaiglm), 0)) / norm(x_h2oaiglm);
results.avg_violation = mean(abs(min(b - A * x_h2oaiglm, 0)));
