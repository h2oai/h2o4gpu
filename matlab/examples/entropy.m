...
%%ENROPY Test H2OGPUML on an entropy maximization problem.
...
results.max_violation = max(abs(sum(x_h2ogpuml) - 1), ...
                            max(max(F * x_h2ogpuml - b, 0)));
results.avg_violation = mean(max(F * x_h2ogpuml - b, 0));
