...
%%ENROPY Test H2OAIGLM on an entropy maximization problem.
...
results.max_violation = max(abs(sum(x_h2oaiglm) - 1), ...
                            max(max(F * x_h2oaiglm - b, 0)));
results.avg_violation = mean(max(F * x_h2oaiglm - b, 0));
