...
dotest:
	...
	mkdir -p ./tmp/
  # can't do -n auto due to limits on GPU memory
	pytest -s --verbose --durations=10 --numprocesses 64 --count 1000000 --fulltrace --full-trace --junit-xml=build/test-reports/h2o4gpu-test.xml tests/python/open_data/system/test_import.py 2> ./tmp/h2o4gpu-test.$(LOGEXT).log
