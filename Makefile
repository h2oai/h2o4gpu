...
docker-runtime-nccl-cuda9:
	...
	export versionTag="0.2.0" ;\
	export extratag="-nccl-cuda9" ;\
	export encodedFullVersionTag=0.2.0 ;\
	export fullVersionTag=0.2.0 ;\
...
get_docker-nccl-cuda9:
	wget https://s3.amazonaws.com/h2o-release/h2o4gpu/releases/bleeding-edge/ai/h2o/h2o4gpu/0.2-nccl-cuda9/h2o4gpu-0.2.0-nccl-cuda9-runtime.tar.bz2
...
docker-runtime-nccl-cuda8-load:
	pbzip2 -dc h2o4gpu-0.2.0-nccl-cuda8-runtime.tar.bz2 | nvidia-docker load
...
get_docker-nccl-cuda8:
	wget https://s3.amazonaws.com/h2o-release/h2o4gpu/releases/bleeding-edge/ai/h2o/h2o4gpu/0.2-nccl-cuda8/h2o4gpu-0.2.0-nccl-cuda8-runtime.tar.bz2

run_in_docker-nccl-cuda8:
	mkdir -p log ; nvidia-docker run --name localhost --rm -p 8888:8888 -u `id -u`:`id -g` -v `pwd`/log:/log --entrypoint=./run.sh opsh2oai/h2o4gpu-0.2.0-nccl-cuda8-runtime &
