                ...
                script {
                    try {
                        sh """
                            ...
                            nvidia-docker exec h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID ln -s /data ./data
                            nvidia-docker exec h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID rm -rf py3nvml
                            nvidia-docker exec h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID bash -c '. /h2oai_env/bin/activate; pip install `find src/interface_py/dist -name "*h2o4gpu*.whl"`; make dotestbig'
                        """
                    } finally {
                        sh """
                            nvidia-docker stop h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID
                ...
                script {
                    try {
                        sh """
                            ...
                            nvidia-docker exec h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID ln -s /data ./data
                            nvidia-docker exec h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID rm -rf py3nvml
                            nvidia-docker exec h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID bash -c '. /h2oai_env/bin/activate; pip install `find src/interface_py/dist -name "*h2o4gpu*.whl"`; make dotestbigperf'
                        """
                    } finally {
                        sh """
                            nvidia-docker stop h2o4gpu${SAFE_CHANGE_ID}-$BUILD_ID
