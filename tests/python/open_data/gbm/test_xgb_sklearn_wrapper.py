...
def test_drf_regressor_backupsklearn(backend='auto'):
    ...
    drf_sk = RandomForestRegressorSklearn(
        random_state=1234, oob_score=True, max_depth=3, n_estimators=10, n_jobs=-1)
    print("Scikit fit()")
    drf_sk.fit(X, y)
    ...
    if backend == "sklearn":
        ...
        assert (drf.decision_path(X)[1] ==
                drf_sk.decision_path(X)[1]).all() == True
        ...
        assert (drf.feature_importances_ ==
                drf_sk.feature_importances_).all() == True
...
def test_drf_classifier_backupsklearn(backend='auto'):
    ...
    drf_sk = RandomForestClassifierSklearn(
        random_state=1234, oob_score=True, max_depth=3, n_estimators=10, n_jobs=-1)
    print("Scikit fit()")
    drf_sk.fit(X, y)
    ...
    if backend == "sklearn":
        assert (drf.predict(X) == drf_sk.predict(X)).all() == True
        assert np.allclose(drf.predict_log_proba(X),
                           drf_sk.predict_log_proba(X))
        assert np.allclose(drf.predict_proba(X), drf_sk.predict_proba(X))
        assert (drf.score(X, y) == drf_sk.score(X, y)).all() == True
        assert (drf.decision_path(X)[1] ==
                drf_sk.decision_path(X)[1]).all() == True
        assert (drf.apply(X) == drf_sk.apply(X)).all() == True
...
def test_gbm_regressor_backupsklearn(backend='auto'):
    ...
    if backend == "sklearn":
        ...
        assert (gbm.feature_importances_ ==
                gbm_sk.feature_importances_).all() == True
...
def test_gbm_classifier_backupsklearn(backend='auto'):
    ...
    if backend == "sklearn":
        assert (gbm.predict(X) == gbm_sk.predict(X)).all() == True
        assert (gbm.predict_log_proba(X) ==
                gbm_sk.predict_log_proba(X)).all() == True
        assert (gbm.predict_proba(X) == gbm_sk.predict_proba(X)).all() == True
        assert (gbm.score(X, y) == gbm_sk.score(X, y)).all() == True
        assert (gbm.decision_function(X)[
                1] == gbm_sk.decision_function(X)[1]).all() == True
        assert np.allclose(list(gbm.staged_predict(X)),
                           list(gbm_sk.staged_predict(X)))
...
def gbm_regressor_parameter_fallback(**kwargs):
    df = pd.read_csv("./open_data/simple.txt", delim_whitespace=True)
    X = np.array(df.iloc[:, :df.shape[1] - 1], dtype='float32', order='C')
    y = np.array(df.iloc[:, df.shape[1] - 1], dtype='float32', order='C')
    import h2o4gpu

    gbm = h2o4gpu.GradientBoostingRegressor(**kwargs)
    print("h2o4gpu fit()")
    gbm.fit(X, y)

    from h2o4gpu.ensemble import GradientBoostingRegressorSklearn
    gbm_sk = GradientBoostingRegressorSklearn(**kwargs)
    print("Scikit fit()")
    gbm_sk.fit(X, y)

    assert np.allclose(gbm.predict(X), gbm_sk.predict(X))

def test_gbm_regressor_parameter_loss_alpha_issue_806():
    df = pd.read_csv("./open_data/simple.txt", delim_whitespace=True)
    X = np.array(df.iloc[:, :df.shape[1] - 1], dtype='float32', order='C')
    y = np.array(df.iloc[:, df.shape[1] - 1], dtype='float32', order='C')
    import h2o4gpu
    ...
    gbm_sk = h2o4gpu.GradientBoostingRegressor(loss='quantile', alpha=0.9)

    gbm_sk.fit(X, y)

    assert not np.allclose(gbm.predict(X), gbm_sk.predict(X))
...
def test_sklearn_gbm_regression_sklearn(
...
def test_gbm_regressor_parameter_fallback_loss() : gbm_regressor_parameter_fallback(loss='quantile')
