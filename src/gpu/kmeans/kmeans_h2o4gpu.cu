...
#pragma omp parallel for
    for (int i = 0; i < num_gpu; i++) {
      try {
        ...
        std::cerr << "Exception within OpenMP parallel section:" << e.what()
                  << '\n';
        throw;
      }
    }
    ...
    for (int i = 0; i < num_gpu; i++) {
      try {
        ...
        if (pot_cent_num > 0) {
          ...
          thrust::device_vector<T> d_new_potential_centroids(pot_cent_num *
                                                             cols);
...
#pragma omp parallel for
    for (int q = 0; q < n_gpu; q++) {
      CUDACHECK(cudaSetDevice(dList[q]));
      CUDACHECK(cudaMemcpy(thrust::raw_pointer_cast(&(*d_centroids[q])[0]),
