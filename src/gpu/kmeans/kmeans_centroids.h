...
template <typename T>
__device__ __forceinline__ void update_centroid(int label, int dimension, int d,
...
__global__ void calculate_centroids(int n, int d, int k, T *data,
  ...
  for (int dimension = threadIdx.x; dimension < d; dimension += blockDim.x) {
    ...
    if (start < n) {
      ...
      update_centroid(prior_label, dimension, d, accumulator, centroids, count,
                      counts);
    }
  }
}
...
__global__ void revert_zeroed_centroids(int d, int k, T *tmp_centroids,
  ...
  if ((global_id_x < d) && (global_id_y < k)) {
    if (counts[global_id_y] < 1) {
      centroids[global_id_x + d * global_id_y] =
          tmp_centroids[global_id_x + d * global_id_y];
    }
  }
}
...
template <typename T>
void find_centroids(int q, int n, int d, int k, thrust::device_vector<T> &data,
                    thrust::device_vector<int> &labels,
                    thrust::device_vector<T> &centroids,
                    thrust::device_vector<int> &range,
                    thrust::device_vector<int> &indices,
                    thrust::device_vector<int> &counts, bool scale) {
  int dev_num;
  ...
  // Need to zero this - the algo uses this array to accumulate values for
  // each centroid which are then averaged to get a new centroid
  memzero(centroids);
  memzero(counts);
  ...
  calculate_centroids<<<dim3(1, 30), dim3(n_threads_x, n_threads_y),  // TODO
                                                                      // FIXME
                        0, cuda_stream[dev_num]>>>(
      n, d, k, thrust::raw_pointer_cast(data.data()),
  ...
  // move centroids and counts between host and device all the time for
  // nothing
  if (scale) {
    // Revert only if `scale`, otherwise this will be taken care of in the
    // host Revert reverts centroids for which count is equal 0
    revert_zeroed_centroids<<<dim3((d - 1) / 32 + 1, (k - 1) / 32 + 1),
                              dim3(32, 32),  // TODO FIXME
                              0, cuda_stream[dev_num]>>>(
        d, k, thrust::raw_pointer_cast(tmp_centroids.data()),
        thrust::raw_pointer_cast(centroids.data()),
        thrust::raw_pointer_cast(counts.data()));
    ...
    scale_centroids<<<dim3((d - 1) / 32 + 1, (k - 1) / 32 + 1),
                      dim3(32, 32),  // TODO FIXME
                      0, cuda_stream[dev_num]>>>(
        d, k, thrust::raw_pointer_cast(counts.data()),
        thrust::raw_pointer_cast(centroids.data()));
#if (CHECK)
    gpuErrchk(cudaGetLastError());
#endif
