...
int kmeans(int verbose, volatile std::atomic_int *flag, int n, int d, int k,
           ...
           thrust::device_vector<T> **data_dots, std::vector<int> dList,
           int n_gpu, int max_iterations, double threshold = 1e-3,
...
#pragma omp parallel for
  for (int q = 0; q < n_gpu; q++) {
    ...
    try {
      ...
      log_error(verbose,
                "Unable to allocate memory for gpu: %d | n/n_gpu: %d | "
                "k: %d | d: %d | error: %s",
...
#pragma omp parallel for
    for (int q = 0; q < n_gpu; q++) {
      ...
      detail::batch_calculate_distances(
          verbose, q, n / n_gpu, d, k, *data[q], *centroids[q], *data_dots[q],
          *centroid_dots[q],
          [&](int n, size_t offset,
      ...
      detail::find_centroids(q, n / n_gpu, d, k, *data[q], *labels_copy[q],
                             *centroids[q], *range[q], *indices[q], *counts[q],
                             n_gpu <= 1);
    }
...
#pragma omp parallel for
    ...
    if (do_per_iter_check) {
      ...
      T squared_norm = thrust::inner_product(
          ...
          (*centroids[0]).begin(), (T)0.0, thrust::plus<T>(),
