...
inline cublasStatus_t throw_on_cublas_error(cublasStatus_t status,
...
inline cusolverStatus_t throw_on_cusolver_error(cusolverStatus_t status,
                                                const char* file, int line) {
  if (status != CUSOLVER_STATUS_SUCCESS) {
    printf("cusolve status %d\n", int(status));
    std::stringstream ss;
...
inline double clocks_to_s(clock_t t) { return (double)t / CLOCKS_PER_SEC; }

struct sqr_op {
  __device__ float operator()(float val) const { return val * val; };
};

struct sqrt_op {
  __device__ float operator()(float val) const { return sqrt(val); };
};
...
inline void generate_column_segments(
    thrust::device_vector<int>& column_segments, int col_size) {
  auto counting = thrust::make_counting_iterator(0);
  thrust::transform(counting, counting + column_segments.size(),
                    column_segments.begin(),
                    [=] __device__(int idx) { return idx * col_size; });
}
}  // namespace h2o4gpu
