...
{
    updateXWithCGKernel3<<<batchSize, f, (4 * f + 4) * sizeof(float)>>>((half *)A, x, b, batchSize, f, cgIter);
...
__global__ void RMSE(const float *csrVal,
                     const int *cooRowIndex,
                     const int *csrColIndex,
                 ...
                 const float *__restrict__ thetaT)
{
    ...
    if (row < m)
    {
        ...
        for (int iter = 0; iter < iterations; iter++)
        {
            ...
            { //this redundant "if" seems improving kernel performance
                for (int k = 0; k < SCAN_BATCH; k++)
                {
                    accumulate_in_registers();
                }
...
{
  public:
    ALSFactorization(const int m, const int n, const int f,
                     const T lambda, T *thetaTDevice, T *XTDevice) : m(m), n(n), f(f), lambda(lambda),
                                                                     thetaT(thetaTDevice), XT(XTDevice)
    {
...
// saveDeviceFloatArrayToFile(std::string("./log/0827/lu-xx32.iter") + std::to_string(iter) + std::string(".batch") + std::to_string(batch_id),  f * f * batch_size, xx);
    ...
    T Score(const int *cooRowIndex, const int *cooColIndex, const T *cooData, const long nnz)
    {
        ...
        RMSE<<<(nnz + block - 1) / block, block>>>(cooData, cooRowIndex, cooColIndex, thetaT, XT,
...
{
    alsUpdateFeature100<<<m, 64, SCAN_BATCH * F / 2 * sizeof(float2)>>>(batch_offset, csrRowIndex, csrColIndex, lambda, m, F, thetaT, XT, ythetaT, cgIter);
    cudaDeviceSynchronize();
    cudaCheckError();
}
