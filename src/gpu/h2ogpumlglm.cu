...
H2OGPUML<T, M, P>::H2OGPUML(int sharedA, int me, int wDev, const M &A)
...
template <typename T, typename M, typename P>
H2OGPUML<T, M, P>::H2OGPUML(const M &A)
...
template <typename T, typename M, typename P>
H2OGPUMLStatus H2OGPUML<T, M, P>::Solve(const std::vector<FunctionObj<T> > &f,
                                const std::vector<FunctionObj<T> > &g) {

  //  PUSH_RANGE("H2OGPUMLSolve",H2OGPUMLSolve,1);
...
//    PUSH_RANGE("Init2",Init2,1);
  ...
  //
  // H2OGPUML paper and code:
  // http://foges.github.io/h2ogpuml/ and http://stanford.edu/~boyd/papers/h2ogpuml.html
  ...
  // Extract values from h2ogpuml_data
  PUSH_RANGE("H2OGPUMLExtract",H2OGPUMLExtract,3);
  size_t m = _A.Rows();
  size_t mvalid = _A.ValidRows();
  size_t n = _A.Cols();
...
//  const T kRhoMax     = static_cast<T>(1e4); // upper range for adaptive rho
  ...
  POP_RANGE("H2OGPUMLExtract",H2OGPUMLExtract,3);

  PUSH_RANGE("H2OGPUMLAlloc",H2OGPUMLAlloc,4);
  ...
  CUDA_CHECK_ERR();
  POP_RANGE("H2OGPUMLAlloc",H2OGPUMLAlloc,4);

  
  PUSH_RANGE("H2OGPUMLScale",H2OGPUMLScale,5);
  ...
  CUDA_CHECK_ERR();
  POP_RANGE("H2OGPUMLScale",H2OGPUMLScale,5);
...
//printData(std::cout); //only works for data in host memory!
  ...
  // Check status
  H2OGPUMLStatus status;
  if (!converged && k == _max_iter - 1)
    status = H2OGPUML_MAX_ITER;
  else if (!converged && k < _max_iter - 1)
    status = H2OGPUML_NAN_FOUND;
  else
    status = H2OGPUML_SUCCESS;
  ...
  if (_verbose > 0) {
    Printf(__HBAR__
        ...
        "Iter  : %u\n",
        H2OGPUMLStatusString(status).c_str(), _time, time_init, k);
    Printf(__HBAR__
        "Error Metrics:\n"
        "Pri: "
  ...
  //  POP_RANGE("H2OGPUMLSolve",H2OGPUMLSolve,1);

  return status;
}

template <typename T, typename M, typename P>
int H2OGPUML<T, M, P>::_Init_Predict() {
  DEBUG_EXPECT(!_done_init);
  if (_done_init)
    return 1;
  _done_init = true;
...
template <typename T, typename M, typename P>
void H2OGPUML<T, M, P>::ResetX(void) {
  if (!_done_init)
    _Init();
  CUDACHECK(cudaSetDevice(_wDev));
  ...
  DEBUG_FPRINTF(stderr,"in h2ogpuml ResetX: m=%d n=%d\n",(int)m,(int)n);
...
template class H2OGPUML<double, MatrixDense <double>, ProjectorDirect<double, MatrixDense <double> > >;
template class H2OGPUML<double, MatrixDense <double>, ProjectorCgls<double,   MatrixDense <double> > >;
template class H2OGPUML<double, MatrixSparse<double>, ProjectorCgls<double,   MatrixSparse<double> > >;
#endif

#if !defined(H2OGPUML_SINGLE) || H2OGPUML_SINGLE==1
template class H2OGPUML<float, MatrixDense<float>,  ProjectorDirect<float, MatrixDense<float> > >;
template class H2OGPUML<float, MatrixDense<float>,  ProjectorCgls<float, MatrixDense<float> > >;
template class H2OGPUML<float, MatrixSparse<float>, ProjectorCgls<float, MatrixSparse<float> > >;
#endif
