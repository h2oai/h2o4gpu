...
//  const T kRhoMax     = static_cast<T>(1e4); // upper range for adaptive rho
	...
	CUDA_CHECK_ERR(); POP_RANGE("H2OGPUMLAlloc",H2OGPUMLAlloc,4);
	...
	CUDA_CHECK_ERR(); POP_RANGE("H2OGPUMLScale",H2OGPUMLScale,5);
...
//printData(std::cout); //only works for data in host memory!
	...
	if (_verbose > 0) {
		...
		"Iter  : %u\n", H2OGPUMLStatusString(status).c_str(), _time, time_init,
				k);
		Printf(
				__HBAR__
				"Error Metrics:\n"
