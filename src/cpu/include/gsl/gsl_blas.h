...
template <>
inline void blas_axpy(double alpha, const vector<double> *x, vector<double> *y) {
  cblas_daxpy(static_cast<int>(x->size), alpha, x->data,
      static_cast<int>(x->stride), y->data, static_cast<int>(y->stride));
}
template <>
inline void blas_axpy(float alpha, const vector<float> *x, vector<float> *y) {
...
// Nrm2.
template <typename T>
T blas_nrm2(const vector<T> *x);
template <>
inline double blas_nrm2(const vector<double> *x) {
  return cblas_dnrm2(static_cast<int>(x->size), x->data,
      static_cast<int>(x->stride));
}

template <>
inline float blas_nrm2(const vector<float> *x) {
...
template <>
inline void blas_scal(const double alpha, vector<double> *x) {
  cblas_dscal(static_cast<int>(x->size), alpha, x->data,
     static_cast<int>(x->stride));
}
template <>
inline void blas_scal(const float alpha, vector<float> *x) {
...
template <>
inline double blas_asum(const vector<double> *x) {
  return cblas_dasum(static_cast<int>(x->size), x->data,
      static_cast<int>(x->stride));
}
template <>
inline float blas_asum(const vector<float> *x) {
...
template <>
inline void blas_dot(const vector<double> *x, const vector<double> *y,
              double *result) {
  *result = cblas_ddot(static_cast<int>(x->size), x->data,
     static_cast<int>(x->stride), y->data, static_cast<int>(y->stride));
}
template <>
inline void blas_dot(const vector<float> *x, const vector<float> *y,
              float *result) {
...
void blas_gemv_(CBLAS_TRANSPOSE_t TransA, double alpha,
                const matrix<double, O> *A, const vector<double> *x,
...
template <CBLAS_ORDER O>
void blas_gemv_(CBLAS_TRANSPOSE_t TransA, float alpha,
                const matrix<float, O> *A, const vector<float> *x,
...
void blas_gemv(CBLAS_TRANSPOSE_t TransA, T alpha,
               const  matrix<T, O> *A, const vector<T> *x, T beta,
               vector<T> *y) {
