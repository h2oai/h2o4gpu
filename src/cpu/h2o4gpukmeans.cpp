...
template <typename T>
void nonrandom_data(int verbose, const char ord, std::vector<T> &array,
...
template <typename T>
void random_centroids(int verbose, const char ord, std::vector<T> &array,
                      const T *srcdata, int q, int n, int npercpu, int d,
                      int k)
{
    std::random_device
...
template <typename T>
void random_centroids_new(int verbose, std::vector<int> v, const char ord,
                          std::vector<T> &array, const T *srcdata, int q, int n,
                          int npercpu, int d, int k)
{
    if (ord == 'c')
    {
        if (VERBOSE)
...
inline void my_function(int sig)
...
template <typename T>
int kmeans_fit(int verbose, int seed, int cpu_idtry, int n_cputry, size_t rows,
               ...
               int **pred_labels)
{
    ...
    for (int q = 0; q < n_cpu; q++)
    {
        nonrandom_data(verbose, 'r', *data[q], &srcdata[0], q, n, n / n_cpu, d);
    }
    ...
    kmeans::kmeans<T>(verbose, &flag, n, d, k, *data[masterq], *labels[masterq],
                      *l_centroids[masterq], max_iterations, init_from_data,
                      threshold);
...
template <typename T>
int kmeans_predict(int verbose, int cpu_idtry, int n_cputry, size_t rows,
                   ...
                   int **pred_labels)
{
    ...
    for (int q = 0; q < n_cpu; q++)
    {
        ...
        nonrandom_data(verbose, 'r', *data[q], &srcdata[0], q, n, n / n_cpu, m);
        ...
        kmeans::compute_distances(*data[q], *data_dots[q], n / n_cpu, m,
                                  *l_centroids, *centroid_dots[q], k,
                                  *pairwise_distances[q]);
...
template <typename T>
int kmeans_transform(int verbose, int gpu_id, int n_gpu, size_t rows,
                     size_t cols, int k, const T *srcdata, const T *centroids,
                     T **preds)
{
    if (rows > std::numeric_limits<int>::max())
...
template <typename T>
int makePtr_dense(int dopredict, int verbose, int seed, int cpu_idtry,
                  int n_cputry, size_t rows, size_t cols, int k,
                  ...
                  int **pred_labels)
{
    if (dopredict == 0)
    {
        return kmeans_fit(verbose, seed, cpu_idtry, n_cputry, rows, cols, k,
                          max_iterations, init_from_data, threshold, srcdata,
                          pred_centroids, pred_labels);
    }
    else
    {
        return kmeans_predict(verbose, cpu_idtry, n_cputry, rows, cols, k, srcdata,
                              centroids, pred_labels);
    }
}

template int makePtr_dense<float>(int dopredict, int verbose, int seed,
                                  int cpu_idtry, int n_cputry, size_t rows,
...
template int kmeans_fit<float>(int verbose, int seed, int cpu_idtry,
                               int n_cputry, size_t rows, size_t cols, int k,
                               int max_iterations, int init_from_data,
                               float threshold, const float *srcdata,
                               float **pred_centroids, int **pred_labels);

template int kmeans_fit<double>(int verbose, int seed, int cpu_idtry,
                                int n_cputry, size_t rows, size_t cols, int k,
                                int max_iterations, int init_from_data,
                                double threshold, const double *srcdata,
                                double **pred_centroids, int **pred_labels);

template int kmeans_predict<float>(int verbose, int cpu_idtry, int n_cputry,
                                   size_t rows, size_t cols, int k,
                                   const float *srcdata, const float *centroid,
                                   int **pred_labels);

template int kmeans_predict<double>(int verbose, int cpu_idtry, int n_cputry,
                                    size_t rows, size_t cols, int k,
                                    const double *srcdata,
                                    const double *centroid, int **pred_labels);
...
template class H2O4GPUKMeansCPU<double>;

#endif

#if !defined(H2O4GPU_SINGLE) || H2O4GPU_SINGLE == 1

template class H2O4GPUKMeansCPU<float>;
...
int make_ptr_float_kmeans(int dopredict, int verbose, int seed, int cpu_id,
                          int n_cpu, size_t mTrain, size_t n, int k,
                          int max_iterations, int init_from_data,
                          float threshold, const float *srcdata,
                          ...
                          int **pred_labels)
{
    return h2o4gpukmeans::makePtr_dense<float>(
        dopredict, verbose, seed, cpu_id, n_cpu, mTrain, n, k, max_iterations,
        init_from_data, threshold, srcdata, centroids, pred_centroids,
        pred_labels);
}

int make_ptr_double_kmeans(int dopredict, int verbose, int seed, int cpu_id,
                           int n_cpu, size_t mTrain, size_t n, int k,
                           int max_iterations, int init_from_data,
                           double threshold, const double *srcdata,
