...
static const std::string H2OGPUML_VERSION = "0.3.2";
...
// Status messages
enum H2OGPUMLStatus { H2OGPUML_SUCCESS,    // Converged successfully.
...
class H2OGPUML {
 ...
 public:
  ...
  H2OGPUML(int sharedA, int me, int wDev, const M &A);
  H2OGPUML(const M &A);
  ~H2OGPUML();

  // Solve for specific objective.
  H2OGPUMLStatus Solve(const std::vector<FunctionObj<T> >& f,
...
using H2OGPUMLDirect = H2OGPUML<T, M, ProjectorDirect<T, M> >;

template <typename T, typename M>
using H2OGPUMLIndirect = H2OGPUML<T, M, ProjectorCgls<T, M> >;
#endif

// String version of status message.
inline std::string H2OGPUMLStatusString(H2OGPUMLStatus status) {
  switch(status) {
    case H2OGPUML_SUCCESS:
      return "Solved";
    case H2OGPUML_UNBOUNDED:
      return "Unbounded";
    case H2OGPUML_INFEASIBLE:
      return "Infeasible";
    case H2OGPUML_MAX_ITER:
      return "Reached max iter";
    case H2OGPUML_NAN_FOUND:
