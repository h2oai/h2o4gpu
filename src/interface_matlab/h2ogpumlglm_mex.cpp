...
int PopulateFunctionObj(const char fn_name[], const mxArray *f_mex,
                        ...
                        std::vector<FunctionObj<T> > *f_h2ogpuml) {
...
#pragma unroll
  ...
  if (param_idx[0] == -1) {
    mexErrMsgIdAndTxt("MATLAB:h2ogpuml:missingParam",
  ...
  for (unsigned int i = 0; i < kNumParam; ++i) {
    if (param_idx[i] != -1) {
      ...
      if (mxGetM(arr) == 1 && mxGetN(arr) == 1) {
        ...
        mexErrMsgIdAndTxt("MATLAB:h2ogpuml:dimensionMismatch",
...
#pragma unroll
    ...
    f_h2ogpuml->push_back(FunctionObj<T>(func_param, real_params[0], real_params[1],
        real_params[2], real_params[3], real_params[4]));
  }
  return 0;
}

// Populate parameters (rel_tol, abs_tol, max_iter, rho and quiet) in H2OGPUMLData.
template <typename T, typename M, typename P>
int PopulateParams(const mxArray *params, h2ogpuml::H2OGPUML<T, M, P> *h2ogpuml_data) {
  // Check if parameter exists in params, then make sure that it has
  // dimension 1x1 and finally set the corresponding value in h2ogpuml_data.
  ...
  if (rel_tol_idx != -1) {
    ...
    h2ogpuml_data->SetRelTol(GetVal<T>(mxGetData(arr), 0, mxGetClassID(arr)));
  }
  ...
  if (abs_tol_idx != -1) {
    ...
    h2ogpuml_data->SetAbsTol(GetVal<T>(mxGetData(arr), 0, mxGetClassID(arr)));
  }
  ...
  if (rho_idx != -1) {
    ...
    h2ogpuml_data->SetRho(GetVal<T>(mxGetData(arr), 0, mxGetClassID(arr)));
  }
  ...
  if (quiet_idx != -1) {
    ...
    h2ogpuml_data->SetVerbose(GetVal<int>(mxGetData(arr), 0, mxGetClassID(arr)));
  }
  ...
  if (gap_stop_idx != -1) {
    ...
    h2ogpuml_data->SetGapStop(GetVal<bool>(mxGetData(arr), 0, mxGetClassID(arr)));
  }
...
// Wrapper for graph h2ogpuml. Populates h2ogpuml_data structure and calls h2ogpuml.
template <typename T>
void SolverWrapDn(int wDev, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  int sharedA=0;
  int me=0;
  h2ogpuml::MatrixDense<T> A_(sharedA, me, wDev,'c', m, n, reinterpret_cast<T*>(mxGetData(prhs[0])));
  ...
  // Populate parameters.
  if (!err && nrhs == 4)
    err = PopulateParams(prhs[3], &h2ogpuml_data);
  ...
  for (unsigned int i = 0; i < num_obj && !err; ++i) {
    ...
    // Run solver.
    h2ogpuml::H2OGPUMLStatus status = h2ogpuml_data.Solve(f, g);

    // Get solution.
    memcpy(reinterpret_cast<T*>(mxGetData(plhs[0])) + i * n, h2ogpuml_data.GetX(),
        n * sizeof(T));
    if (nlhs >= 2) {
      memcpy(reinterpret_cast<T*>(mxGetData(plhs[1])) + i * m,
          h2ogpuml_data.GetY(), m * sizeof(T));
    }
    if (nlhs >= 3) {
      memcpy(reinterpret_cast<T*>(mxGetData(plhs[2])) + i * m,
          h2ogpuml_data.GetLambda(), m * sizeof(T));
    }
    if (nlhs >= 4) {
      memcpy(reinterpret_cast<T*>(mxGetData(plhs[3])) + i * n,
          h2ogpuml_data.GetMu(), n * sizeof(T));
    }

    if (nlhs >= 5)
      reinterpret_cast<T*>(mxGetData(plhs[4]))[i] = h2ogpuml_data.GetOptval();
    if (nlhs >= 6)
...
void SolverWrapSp(int wDev, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  H2OGPUML_INT *row_ind = new H2OGPUML_INT[nnz];
  H2OGPUML_INT *col_ptr = new H2OGPUML_INT[n + 1];
  IntToInt(nnz, mw_row_ind, row_ind);
  IntToInt(n + 1, mw_col_ptr, col_ptr);

  // Initialize H2OGPUML data structure
  h2ogpuml::MatrixSparse<T> A(wDev, 'c', m, n, nnz, val, col_ptr, row_ind);
  h2ogpuml::H2OGPUMLIndirect<T, h2ogpuml::MatrixSparse<T> > h2ogpuml_data(A);
  std::vector<FunctionObj<T> > f;
  std::vector<FunctionObj<T> > g;
...
void mexFunction(int wDev, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs < 3 || nrhs > 4) {
    mexErrMsgIdAndTxt("MATLAB:h2ogpuml:insufficientInputArgs",
        "Usage: [x, y, l, u, optval, status] = h2ogpuml(A, f, g, [params])");
    return;
  }
  if (nlhs > 6) {
    mexErrMsgIdAndTxt("MATLAB:h2ogpuml:extraneousOutputArgs",
        "Usage: [x, y, l, u, optval, status] = h2ogpuml(A, f, g, [params])");
    return;
  }
  ...
  if (class_id_A != mxSINGLE_CLASS && class_id_A != mxDOUBLE_CLASS) {
    mexErrMsgIdAndTxt("MATLAB:h2ogpuml:inputNotNumeric",
        "Matrix A must either be single or double precision.");
    return;
  }
  if (mxGetClassID(prhs[1]) != mxSTRUCT_CLASS) {
    mexErrMsgIdAndTxt("MATLAB:h2ogpuml:inputNotStruct",
