...
void PopulateFunctionObj(SEXP f, unsigned int n,
                         std::vector<FunctionObj<double> > *f_h2oaiglm) {
  ...
  for (unsigned int i = 0; i < n; ++i) {
    ...
    (*f_h2oaiglm)[i]=FunctionObj<double>(func_param, real_params[0],
...
//  std::copy(REAL(A), REAL(A) + m*n, std::ostream_iterator<T>(std::cout, "\n"));
  ...
  // Populate parameters.
  PopulateParams(params, &h2oaiglm_data); //also sets wDev again
  ...
  for (unsigned int i = 0; i < num_obj && !err; ++i) {
    ...
    // Run solver.
    INTEGER(status)[i] = h2oaiglm_data.Solve(f, g);

    // Get Solution
    memcpy(REAL(x) + i * n, h2oaiglm_data.GetX(), n * sizeof(T));
    memcpy(REAL(y) + i * m, h2oaiglm_data.GetY(), m * sizeof(T));
    memcpy(REAL(u) + i * n, h2oaiglm_data.GetMu(), n * sizeof(T));
