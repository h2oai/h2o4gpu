...
#' @import Matrix
#' @export  
h2oaiglm <- function(A, f, g, params=list()) {
  # Make sure A is numeric matrix.
  if (!is.numeric(A)) {
    stop("h2oaiglm(): input A must be numeric!")
  }
  if (!is.matrix(A)) {
    stop("h2oaiglm(): input A must be a matrix!")
  }

  # Make sure f and g are lists.
  if (!is.list(f) || !is.list(g) || !is.list(params)) {
    stop("h2oaiglm(): inputs f, g and params must be lists!")
  }
  ...
  for (i in 1:length(f)) {
    ...
    if (!any(names(fi) == "h")) {
      stop("h2oaiglm(): field f$h is required!")
    }
    for (name in names(fi)) {
      if (!any(name == c("a", "b", "c", "d", "e", "h"))) {
        stop(cat("h2oaiglm(): field f$", name, " unknown!", sep=""))
      }
      if (!is.numeric(fi[[name]])) {
        stop(cat("h2oaiglm(): field f$", name, " must be numeric!", sep=""))
      }
      # Convert to double (in case it's a matrix).
      fi[[name]] = as.double(fi[[name]])
      if (!is.vector(fi[[name]])) {
        stop(cat("h2oaiglm(): field f$", name, " must be a vector!", sep=""))
      }
      if (length(fi[[name]]) != nrow(A) && length(fi[[name]]) != 1) {
        stop(cat("h2oaiglm(): field f$", name, " must must have length nrow(A) or 1!", sep=""))
      }
    }
  }
  ...
  for (i in 1:length(g)) {
    ...
    if (!any(names(gi) == "h")) {
      stop("h2oaiglm(): field g$h is required!")
    }
    for (name in names(gi)) {
      if (!any(name == c("a", "b", "c", "d", "e", "h"))) {
        stop(cat("h2oaiglm(): field g$", name, " unknown!", sep=""))
      }
      if (!is.numeric(gi[[name]])) {
        stop(cat("h2oaiglm(): field g$", name, " must be numeric!", sep=""))
      }
      # Convert to double (in case it's a matrix).
      gi[[name]] = as.double(gi[[name]])
      if (!is.vector(gi[[name]])) {
        stop(cat("h2oaiglm(): field g$", name, " must be a vector!", sep=""))
      }
      if (length(gi[[name]]) != ncol(A) && length(gi[[name]]) != 1) {
        stop(cat("h2oaiglm(): field g$", name, " must must have length ncol(A) or 1!", sep=""))
      }
    }
  }
  ...
  for (name in names(params)) {
    if (!any(name == c("rel_tol", "abs_tol", "rho", "max_iter", "verbose", "adaptive_rho", "equil", "gap_stop", "nDev", "wDev"))) {
      stop(cat("h2oaiglm(): field params$", name, " unknown!", sep=""))
    }
    if (!is.numeric(params[[name]]) && name != "adaptive_rho" && name != "equil" && name != "gap_stop") {
      stop(cat("h2oaiglm(): field params$", name, " must be numeric!", sep=""))
    }
    if (!is.logical(params[[name]]) && (name == "adaptive_rho" || name == "equil" || name == "gap_stop")) {
      stop(cat("h2oaiglm(): field params$", name, " must be logical!", sep=""))
    }
    if (length(params[[name]]) != 1) {
      stop(cat("h2oaiglm(): field params$", name, " must must have length 1!", sep=""))
    }
    if (name == "verbose") {
...
############################################################################################ 
##################################### H2OAIGLMNET FUNCTION #####################################
############################################################################################
#' @title Fit a GLM with lasso or elasticnet regularization using H2OAIGLM
...
h2oaiglmnet <- function(x, y, family=c("gaussian", "binomial"),
  ...
  class(fit) = c("h2oaiglmnet", family)
  fit
}
...
#' @export
predict.h2oaiglmnet <- function(object, newx, s=NULL, type=c("link", "response", "class"), ...) {
  type = match.arg(type)
  if (is.null(s)) {
    beta = object$beta
  } else {
...
#' @title h2oaiglmnet coefficients.
#' @description Returns coeficients of h2oaiglmnet fit.
#' @param object Value returned by h2oaiglmnet(..).
#' @param s Values of lambda for which coefficient should be returned.
#' @param ... Not used. Other arguments to predict.
#' @export
coef.h2oaiglmnet <- function(object, s=NULL, ...) {
  if (is.null(s)) {
    coefs = object$beta
  } else {
    lamlist = lambda.interp(object$lambda, s)
...
#' @title Prints h2oaiglmnet summary
#' @description Prings h2oaiglmnet summary
#' @param x h2oaiglmnet object
#' @param digits Number of digits to display
#' @param ... Not used.
#' @usage \method{print}{h2oaiglmnet}(x, digits = max(3, getOption("digits") - 3), ...)
#' @export
print.h2oaiglmnet <- function(x, digits = max(3, getOption("digits") - 3), ...){
  cat("\nCall: ", deparse(x$call), "\n\n")
...
cv.h2oaiglmnet <- function(x, y, weights, lambda=NULL, nfolds=10, foldid=NULL, cutoff=TRUE, params=list(), ...) {
  ...
  if(!is.null(lambda) && length(lambda) < 2) {
    stop("Need more than one value of lambda for cv.h2oaiglmnet")
  }
  N = nrow(x)
  y = drop(y)
  if(missing(weights)) {
  ...
  # Create initial h2oaiglmnet call (mostly for sequence of lambdas)
  h2oaiglmnet.call = match.call(expand.dots=TRUE)
  h2oaiglmnet.call[[1]] = as.name("h2oaiglmnet") 
  h2oaiglmnet.object = h2oaiglmnet(x, y, weights=weights, lambda=lambda, cutoff=cutoff, params=params, ...)
  h2oaiglmnet.object$call = h2oaiglmnet.call
  lambda = h2oaiglmnet.object$lambda
  ...
  foreach(i=1:nfolds) %dopar% {
    ...
    h2oaiglmnet.fit = h2oaiglmnet(x[!which, , drop=FALSE], y_sub, lambda=lambda, weights=weights[!which], cutoff=FALSE, params=params, ...)
    nlami = length(h2oaiglmnet.fit$lambda)
    predmat[which, seq(nlami)] = predict(h2oaiglmnet.fit, x[which, , drop=FALSE], type="response")
  }
  ...
  out = list(lambda=lambda, cvm=cvm, cvsd=cvsd, cvup=cvm + cvsd, cvlo=cvm - cvsd,
            nzero=h2oaiglmnet.object$df, name="Mean-Squared Error", h2oaiglmnet.fit=h2oaiglmnet.object)
  obj = c(out,as.list(getmin(lambda, cvm, cvsd)))
  class(obj) = "cv.h2oaiglmnet"
  obj
}
...
#' @title Plot h2oaiglmnet CV 
#' @description Plots results of h2oaiglmnet CV
#' @param x h2oaiglmnet.cv object
#' @param sign.lambda Either plot against log(lambda) (default) or its negative if sign.lambda=-1.
#' @param ... Other graphical parameters to plot.
#' @usage \method{plot}{cv.h2oaiglmnet}(x, sign.lambda, ...)
#' @export
plot.cv.h2oaiglmnet <- function(x, sign.lambda=1, ...) {
