...
#' @import Matrix
#' @export  
h2ogpuml <- function(A, f, g, params=list()) {
  # Make sure A is numeric matrix.
  if (!is.numeric(A)) {
    stop("h2ogpuml(): input A must be numeric!")
  }
  if (!is.matrix(A)) {
    stop("h2ogpuml(): input A must be a matrix!")
  }

  # Make sure f and g are lists.
  if (!is.list(f) || !is.list(g) || !is.list(params)) {
    stop("h2ogpuml(): inputs f, g and params must be lists!")
  }
  ...
  for (i in 1:length(f)) {
    ...
    if (!any(names(fi) == "h")) {
      stop("h2ogpuml(): field f$h is required!")
    }
    for (name in names(fi)) {
      if (!any(name == c("a", "b", "c", "d", "e", "h"))) {
        stop(cat("h2ogpuml(): field f$", name, " unknown!", sep=""))
      }
      if (!is.numeric(fi[[name]])) {
        stop(cat("h2ogpuml(): field f$", name, " must be numeric!", sep=""))
      }
      # Convert to double (in case it's a matrix).
      fi[[name]] = as.double(fi[[name]])
      if (!is.vector(fi[[name]])) {
        stop(cat("h2ogpuml(): field f$", name, " must be a vector!", sep=""))
      }
      if (length(fi[[name]]) != nrow(A) && length(fi[[name]]) != 1) {
        stop(cat("h2ogpuml(): field f$", name, " must must have length nrow(A) or 1!", sep=""))
      }
    }
  }
  ...
  for (i in 1:length(g)) {
    ...
    if (!any(names(gi) == "h")) {
      stop("h2ogpuml(): field g$h is required!")
    }
    for (name in names(gi)) {
      if (!any(name == c("a", "b", "c", "d", "e", "h"))) {
        stop(cat("h2ogpuml(): field g$", name, " unknown!", sep=""))
      }
      if (!is.numeric(gi[[name]])) {
        stop(cat("h2ogpuml(): field g$", name, " must be numeric!", sep=""))
      }
      # Convert to double (in case it's a matrix).
      gi[[name]] = as.double(gi[[name]])
      if (!is.vector(gi[[name]])) {
        stop(cat("h2ogpuml(): field g$", name, " must be a vector!", sep=""))
      }
      if (length(gi[[name]]) != ncol(A) && length(gi[[name]]) != 1) {
        stop(cat("h2ogpuml(): field g$", name, " must must have length ncol(A) or 1!", sep=""))
      }
    }
  }
  ...
  for (name in names(params)) {
    if (!any(name == c("rel_tol", "abs_tol", "rho", "max_iter", "verbose", "adaptive_rho", "equil", "gap_stop", "nDev", "wDev"))) {
      stop(cat("h2ogpuml(): field params$", name, " unknown!", sep=""))
    }
    if (!is.numeric(params[[name]]) && name != "adaptive_rho" && name != "equil" && name != "gap_stop") {
      stop(cat("h2ogpuml(): field params$", name, " must be numeric!", sep=""))
    }
    if (!is.logical(params[[name]]) && (name == "adaptive_rho" || name == "equil" || name == "gap_stop")) {
      stop(cat("h2ogpuml(): field params$", name, " must be logical!", sep=""))
    }
    if (length(params[[name]]) != 1) {
      stop(cat("h2ogpuml(): field params$", name, " must must have length 1!", sep=""))
    }
    if (name == "verbose") {
  ...
  # Call H2OGPUML.
  .Call("H2OGPUMLWrapper", A, f, g, params)
}

############################################################################################ 
################################# H2OGPUML OBJECTIVE FUNCTIONS #################################
############################################################################################
...
############################################################################################ 
##################################### H2OGPUMLNET FUNCTION #####################################
############################################################################################
#' @title Fit a GLM with lasso or elasticnet regularization using H2OGPUML
...
#' @export
h2ogpumlnet <- function(x, y, family=c("gaussian", "binomial"),
  ...
  class(fit) = c("h2ogpumlnet", family)
  fit
}
...
#' @title H2OGPUMLnet prediction
#' @description Predict response for new value input
#' @param object Value returned by h2ogpumlnet(..).
...
#' @export
predict.h2ogpumlnet <- function(object, newx, s=NULL, type=c("link", "response", "class"), ...) {
  type = match.arg(type)
  if (is.null(s)) {
    beta = object$beta
  } else {
...
#' @title h2ogpumlnet coefficients.
#' @description Returns coeficients of h2ogpumlnet fit.
#' @param object Value returned by h2ogpumlnet(..).
#' @param s Values of lambda for which coefficient should be returned.
#' @param ... Not used. Other arguments to predict.
#' @export
coef.h2ogpumlnet <- function(object, s=NULL, ...) {
  if (is.null(s)) {
    coefs = object$beta
  } else {
    lamlist = lambda.interp(object$lambda, s)
...
#' @title Prints h2ogpumlnet summary
#' @description Prings h2ogpumlnet summary
#' @param x h2ogpumlnet object
#' @param digits Number of digits to display
#' @param ... Not used.
#' @usage \method{print}{h2ogpumlnet}(x, digits = max(3, getOption("digits") - 3), ...)
#' @export
print.h2ogpumlnet <- function(x, digits = max(3, getOption("digits") - 3), ...){
  cat("\nCall: ", deparse(x$call), "\n\n")
...
#' @param params Other parameters to h2ogpumlnet
#' @param ... Other parameters to h2ogpumlnet.
#' @export
cv.h2ogpumlnet <- function(x, y, weights, lambda=NULL, nfolds=10, foldid=NULL, noweight=FALSE, cutoff=TRUE, params=list(), ...) {
  library(foreach)
  library(doMC)
  ...
  if(!is.null(lambda) && length(lambda) < 2) {
    stop("Need more than one value of lambda for cv.h2ogpumlnet")
  }
  N = nrow(x)
  y = drop(y)
  if(missing(weights)) {
  ...
  # Create initial h2ogpumlnet call (mostly for sequence of lambdas)
  h2ogpumlnet.call = match.call(expand.dots=TRUE)
  h2ogpumlnet.call[[1]] = as.name("h2ogpumlnet") 
  h2ogpumlnet.object = h2ogpumlnet(x, y, weights=weights, lambda=lambda, cutoff=cutoff, params=params, ...)
  h2ogpumlnet.object$call = h2ogpumlnet.call
  lambda = h2ogpumlnet.object$lambda
  ...
  foreach(i=1:nfolds) %dopar% {
    ...
    h2ogpumlnet.fit = h2ogpumlnet(x[!which, , drop=FALSE], y_sub, lambda=lambda, weights=weights[!which], cutoff=FALSE, params=params, ...)
    nlami = length(h2ogpumlnet.fit$lambda)
    predmat[which, seq(nlami)] = predict(h2ogpumlnet.fit, x[which, , drop=FALSE], type="response")
  }
  ...
  out = list(lambda=lambda, cvm=cvm, cvsd=cvsd, cvup=cvm + cvsd, cvlo=cvm - cvsd,
            nzero=h2ogpumlnet.object$df, name="Mean-Squared Error", h2ogpumlnet.fit=h2ogpumlnet.object)
  obj = c(out,as.list(getmin(lambda, cvm, cvsd)))
  class(obj) = "cv.h2ogpumlnet"
  obj
}
...
#' @title Plot h2ogpumlnet CV 
#' @description Plots results of h2ogpumlnet CV
#' @param x h2ogpumlnet.cv object
#' @param sign.lambda Either plot against log(lambda) (default) or its negative if sign.lambda=-1.
#' @param ... Other graphical parameters to plot.
#' @usage \method{plot}{cv.h2ogpumlnet}(x, sign.lambda, ...)
#' @export
plot.cv.h2ogpumlnet <- function(x, sign.lambda=1, ...) {
