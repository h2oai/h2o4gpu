...
#linker options
CXXFLAGS+=$(DEVICEOPTS)  -DH2O4GPU_SINGLE=0
...
$(BUILDPATH)/h2o4gpu.so: $(BUILDPATH)/h2o4gpuglm_r.o ../R/h2o4gpuglm.R $(BUILDPATH)/blas2cblas.o $(BUILDPATH)/h2o4gpuglm_r.o
	mkdir -p $(BUILDPATH)
	$(CXX) -o $@ -shared $(BUILDPATH)/h2o4gpuglm_r.o ../../../$(OBJDIR)/$(TARGETPATH)/h2o4gpu.a \
	$(RPATH) $(LD_FLAGS) $(CXXFLAGS)
	ln -sf $(BUILDPATH)/h2o4gpu.so .
...
$(BUILDPATH)/h2o4gpuglm_r.o: h2o4gpuglm_r.cpp config.mk Makefile
	mkdir -p $(BUILDPATH)
	$(CXX) $(HDR) $(CXXFLAGS) $< -c -o $@

################################################################################
############################## H2O4GPU specific end ###############################
################################################################################
