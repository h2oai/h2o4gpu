...
void PopulateFunctionObj(SEXP f, unsigned int n,
                         std::vector<FunctionObj<double> > *f_h2o4gpu) {
...
//  std::copy(REAL(A), REAL(A) + m*n, std::ostream_iterator<T>(std::cout, "\n"));
  h2o4gpu::MatrixDense<T> A_dense(sharedA, me, wDev, 'c', m, n, REAL(A));

  // Initialize H2O4GPU data structure
  h2o4gpu::H2O4GPUDirect<T, h2o4gpu::MatrixDense<T> > h2o4gpu_data(A_dense);
  ...
  // Populate parameters.
  PopulateParams(params, &h2o4gpu_data); //also sets wDev again
  ...
  for (unsigned int i = 0; i < num_obj && !err; ++i) {
    ...
    // Run solver.
    INTEGER(status)[i] = h2o4gpu_data.Solve(f, g);

    // Get Solution
    memcpy(REAL(x) + i * n, h2o4gpu_data.GetX(), n * sizeof(T));
    memcpy(REAL(y) + i * m, h2o4gpu_data.GetY(), m * sizeof(T));
    memcpy(REAL(u) + i * n, h2o4gpu_data.GetMu(), n * sizeof(T));
    memcpy(REAL(v) + i * m, h2o4gpu_data.GetLambda(), m * sizeof(T));

    REAL(opt)[i] = h2o4gpu_data.GetOptval();
...
extern "C" {
SEXP H2O4GPUWrapper(SEXP A, SEXP f, SEXP g, SEXP params) {
  // Setup output.
