...
This module gathers common declarations needed
 to ensure Python 2 / Python 3 compatibility.
It has to be imported from all other files,
 so that the common header looks like this:
...
-----------------------------------------------------------------
1. Strings
    In Py2 `str` is a byte string (`bytes is str == True`),
     and `unicode` is a unicode string.
    In Py3 `str` is a unicode string, `bytes` is a byte string,
     and symbol `unicode` is not defined.
    Iterating over a `bytes` string in Py2 produces characters,
     in Py3 character codes.

    For consistent results, use
        test_type(s, str)  to test whether an argument is a string
        bytes_iterator(s)  to iterate over byte-codes of strins s
         (which could be bytes or unicode)

2. Integers
    In Py2 there are two integer types: `int` and `long`.
    The latter has suffix "L" when stringified with repr().
...
3. Iterators
    ...
    This module backports these functions into Py2, so that
    they produce iterators both in Py2 and Py3.

4. Dictionaries
    There are multiple inconsistencies between Py2 and Py3
    in the behavior of dict.keys(), dict.values(),
    dict.iteritems() / dict.items(), etc.

    This module installs methods `viewkeys`, `viewvalues`,
     `viewitems` that are preferrable for dict iteration /
    manipulation. These have same semantic as in Py3.

5. Obsolete functions
    Several functions that existed in Py2 were removed in Py3.
     These are:
        apply, cmp, coerce, execfile, file, long, raw_input,
         reduce, reload, unicode, xrange, StandardError

    This module replaces all these functions with stubs
     that produce runtime errors when called. Do not use any of
    these functions if you want your code to be compatible
     across Py2 / Py3!
...
__all__ = ("PY2", "PY3", "with_metaclass", "bytes_iterator", "range", "filter",
           "map", "zip", "viewitems", "viewkeys", "viewvalues", "apply", "cmp",
           "coerce", "execfile", "file", "long", "raw_input", "reduce",
           "reload", "unicode", "xrange", "StandardError", "chr", "input",
           "open", "next", "round", "super", "csv_dict_writer", "repr2")
...
def csv_dict_writer(f, fieldnames, **kwargs):
    """Equivalent of csv.DictWriter,
     but allows `delimiter` to be a unicode string on Py2."""
    import csv
