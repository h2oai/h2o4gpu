...
if not h2ogpumlElasticNetCPU:
    ...
    print('> Setting h2ogpuml.ElasticNetSolverCPU=None')
    print('> Use h2ogpuml.ElasticNetSolverGPU(args...) and re-run setup.py\n\n')
    ...
    class ElasticNetSolverCPU(object):
        def __init__(self, sharedA, nThreads, nGPUs, ord, intercept, standardize, lambda_min_ratio, n_lambdas, n_folds, n_alphas):
            self.solver = ElasticNetBaseSolver(h2ogpumlElasticNetCPU, sharedA, nThreads, nGPUs, ord, intercept, standardize, lambda_min_ratio, n_lambdas, n_folds, n_alphas)
