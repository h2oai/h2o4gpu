	...
	def __init__(self, A, lib, **kwargs):
		try:
			...
			if self.dense and not self.double_precision:
				...
				self.work = self.lib.h2ogpuml_init_dense_double(self.wDev, self.order, self.m, self.n, cptr(A,c_double))
			elif not self.double_precision:
				self.work = self.lib.h2ogpuml_init_sparse_single(self.wDev, self.order, self.m, self.n, A.nnz, cptr(A.data,c_float),
														 cptr(A.indices,c_int), cptr(A.indptr,c_int))
			else:
				self.work = self.lib.h2ogpuml_init_sparse_double(self.wDev, self.order, self.m, self.n, A.nnz, cptr(A.data,c_double),
														 cptr(A.indices,c_int), cptr(A.indptr,c_int))
	...
	def solve(self, f, g, **kwargs):
		try:
			...
			if not self.work:
				...
				self.lib.h2ogpuml_solve_single(self.work, pointer(self.settings), pointer(self.solution), pointer(self.info),
