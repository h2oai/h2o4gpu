...
from h2ogpuml.solvers.elastic_net_base import ElasticNetBaseSolver
from ctypes import *

if not h2ogpumlElasticNetGPU:
    ...
    print('> Setting h2ogpuml.ElasticNetSolverGPU=None')
    print('> Use h2ogpuml.ElasticNetSolverCPU(args...) or add CUDA libraries to $PATH and re-run setup.py\n\n')
    ...
    class ElasticNetSolverGPU(object):
        def __init__(self, sharedA, nThreads, nGPUs, ord, intercept, standardize, lambda_min_ratio, n_lambdas, n_folds, n_alphas):
            self.solver = ElasticNetBaseSolver(h2ogpumlElasticNetGPU, sharedA, nThreads, nGPUs, ord, intercept, standardize, lambda_min_ratio, n_lambdas, n_folds, n_alphas)
