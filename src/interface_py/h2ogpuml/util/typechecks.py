...
:class:`H2OGPUMLTypeError` exception instead of an ``AssertionError``, which is both more precise, and more user-friendly
...
The ``typeI`` items here deserve a more thorough explanation. They could be:
    ...
    assert_is_type(hls, H2OGPUMLLocalServer)
    ...
    assert_is_type(cols, {str: H2OGPUMLFrame})  # Same as Map<str, H2OGPUMLFrame> in Java
...
from h2ogpuml.exceptions import H2OGPUMLTypeError, H2OGPUMLValueError
from h2ogpuml.util.compatibility import *  # NOQA
from h2ogpuml.util.compatibility import PY2, viewitems
...
if PY2:
    ...
    from h2ogpuml.utils.compatibility import _native_unicode, _native_long
...
class _LazyClass(MagicType):
    ...
    ``numpy_ndarray`` (and perhaps others in future). Usage::
        ...
        from h2ogpuml.utils.typechecks import assert_is_type, pandas_dataframe, numpy_ndarray
...
def assert_is_type(var, *types, **kwargs):
    ...
    This function is used to check that the type of the argument is correct, otherwises it raises an H2OGPUMLTypeError.
    ...
    :raises H2OGPUMLTypeError: if the argument is not of the desired type.
    """
    ...
    raise H2OGPUMLTypeError(var_name=vname, var_value=var, var_type_name=vtn, exp_type_name=etn, message=message,
...
def assert_matches(v, regex):
    ...
    if m is None:
        ...
        raise H2OGPUMLValueError(message, var_name=vn, skip_frames=1)
    return m
...
def assert_satisfies(v, cond, message=None):
    ...
    if not cond:
        ...
        raise H2OGPUMLValueError(message=message, var_name=vname, skip_frames=1)
...
def _retrieve_assert_arguments():
    ...
    If the variable `num_threads` turns out to be non-integer, we would like to raise an exception such as
        ...
        H2OGPUMLTypeError("`num_threads` is expected to be integer, but got <str>")
...
def _get_type_name(vtype, dump=None):
    ...
    Return the name of the provided type.
        ...
        _get_type_name(U(H2OGPUMLFrame, None)) == "?H2OGPUMLFrame"
    """
    if vtype is None:
        return "None"
    if vtype is str:
