...
from h2oaiglm.types import ORD, cptr, make_settings, make_solution, make_info, change_settings, change_solution, Solution, FunctionVector
...
class BaseSolver(object):
	def __init__(self, A, lib, **kwargs):
		try:
			...
			assert lib and (lib==h2oaiglmCPU or lib==h2oaiglmGPU)
			...
			if self.dense and not self.double_precision:
				self.work = self.lib.h2oaiglm_init_dense_single(self.wDev, self.order, self.m, self.n, cptr(A,c_float))
			elif self.dense:
				self.work = self.lib.h2oaiglm_init_dense_double(self.wDev, self.order, self.m, self.n, cptr(A,c_double))
			elif not self.double_precision:
				self.work = self.lib.h2oaiglm_init_sparse_single(self.wDev, self.order, self.m, self.n, A.nnz, cptr(A.data,c_float),
														 cptr(A.indices,c_int), cptr(A.indptr,c_int))
			else:
				self.work = self.lib.h2oaiglm_init_sparse_double(self.wDev, self.order, self.m, self.n, A.nnz, cptr(A.data,c_double),
														 cptr(A.indices,c_int), cptr(A.indptr,c_int))
	...
	def init(self, A, lib, **kwargs):
		if not self.work:
			...
			print("H2OAIGLM_work data structure already intialized, cannot re-initialize without calling finish()")
	...
	def solve(self, f, g, **kwargs):
		try:
			...
			if not self.work:
				print("no viable H2OAIGLM_work pointer to call solve(). call Solver.init( args... ) first")
				return 
			elif not self.double_precision:
				self.lib.h2oaiglm_solve_single(self.work, pointer(self.settings), pointer(self.solution), pointer(self.info),
				...
				self.lib.h2oaiglm_solve_double(self.work, pointer(self.settings), pointer(self.solution), pointer(self.info), 
	...
	def finish(self):
		if not self.work:
			print("no viable H2OAIGLM_work pointer to call finish(). call Solver.init( args... ) first")
			pass
		elif not self.double_precision:
			self.lib.h2oaiglm_finish_single(self.work)
			self.work = None
		else:
			self.lib.h2oaiglm_finish_double(self.work)
			self.work = None
		print("shutting down... H2OAIGLM_work freed in C++")
