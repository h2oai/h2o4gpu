...
from pogs_libs import *

#TODO catch Ctrl-C

if not pogsCPU:
	print 'Warning: Cannot create a POGS CPU Solver instance without linking Python module to a compiled POGS CPU libirary\n \
...
if not pogsGPU:
	print 'Warning: Cannot create a POGS GPU Solver instance without linking Python module to a compiled POGS CPU libirary\n \
			Setting pogs.SolverGPU=None'
	SolverGPU=None
else:
	class SolverGPU(object):
		def __init__(self, m, n, A, **kwargs):
			try:
				...
				if self.dense and not self.double_precision:
					self.work = pogsGPU.pogs_init_dense_single(self.order, m, n, cptr(A,c_float))
				elif self.dense:
					self.work = pogsGPU.pogs_init_dense_double(self.order, m, n, cptr(A,c_double))
				elif not self.double_precision:
					self.work = pogsGPU.pogs_init_sparse_single(self.order, m, n, A.nnz, cptr(A.data,c_float), 
															 cptr(A.indices,c_int), cptr(A.indptr,c_int))
				else:
					self.work = pogsGPU.pogs_init_sparse_double(self.order, m, n, A.nnz, cptr(A.data,c_double),
															 cptr(A.indices,c_int), cptr(A.indptr,c_int))
