...
%%LP_EQ Test H2OGPUML on an equality constrained LP.
...
tic
[x_h2ogpuml, ~, ~, n_iter] = h2ogpuml(prox_f, prox_g, obj_fn, [A; c'], params);
time_h2ogpuml = toc;
c' * x_h2ogpuml
...
results.rel_err_obj = ...
    (obj_fn(x_h2ogpuml, [A; c'] * x_h2ogpuml) - cvx_optval) / abs(cvx_optval);
results.rel_diff_soln = norm(x_h2ogpuml - x_cvx) / norm(x_cvx);
results.max_violation = max([abs(b - A * x_h2ogpuml); max(-x_h2ogpuml, 0)]);
results.avg_violation = mean([abs(b - A * x_h2ogpuml); max(-x_h2ogpuml, 0)]);
