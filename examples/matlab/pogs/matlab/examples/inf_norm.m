...
%%INF_NORM Test H2OGPUML on minimizing the L-inf norm.
...
tic
[x_h2ogpuml, ~, ~, n_iter] = h2ogpuml(prox_f, prox_g, obj_fn, B, params);
time_h2ogpuml = toc;
...
results.rel_err_obj = ...
    (obj_fn(x_h2ogpuml, B * x_h2ogpuml) - cvx_optval) / abs(cvx_optval);
results.rel_diff_soln = norm(x_h2ogpuml(1:end - 1) - x_cvx) / norm(x_cvx);
