...
%%PORTFOLIO Test H2OGPUML on a portfolio problem.
...
results.max_violation = max(abs(sum(x_h2ogpuml) - 1), max(max(-x_h2ogpuml, 0))) / ...
    norm(x_h2ogpuml);
results.avg_violation = mean([abs(sum(x_h2ogpuml) - 1); max(-x_h2ogpuml, 0)]) / ...
    norm(x_h2ogpuml);
