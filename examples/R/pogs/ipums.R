...
#    alpha = alpha
    ...
    s2 <- proc.time()
    glmnet = glmnet(x = train_x, y = y, family = family, alpha = alpha, standardize=TRUE, intercept=intercept)
    e2 <- proc.time()
    ...
    print(glmnet$lambda)
    rmse = apply(glmnet_pred_y, 2, function(x) sqrt(mean((x-valid_y)^2)))
    res = as.data.frame(list(rep(alpha,length(glmnet$lambda)), glmnet$lambda, rmse))
    write.csv(x=res,file=paste0("results.glmnet.",alpha),quote=FALSE,row.names=FALSE,col.names=FALSE)
...
if (h2o) {
  ...
  if (FALSE) {
    ...
    s3 <- proc.time()
    h2omodel <- h2o.glm(x=names(train.hex), y=response, training_frame=train.hex, family=family, alpha = alpha, lambda_search=TRUE, solver="COORDINATE_DESCENT")
    e3 <- proc.time()
