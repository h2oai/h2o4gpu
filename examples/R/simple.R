...
model = pogsnet(x = x, y = asthma, family = "gaussian", alpha = alpha, lambda=NULL, lambda.min.ratio=1e-6, intercept=TRUE)
print(paste0("RMSEPOGS:",sqrt(mean((predict(model, x)-asthma)^2))))

library(glmnet)
model = glmnet(x = x, y = asthma, family = "gaussian", alpha = alpha, lambda=NULL)
print(paste0("GLMNETSTDRMSE:",sqrt(mean((predict(model, x)-asthma)^2))))
model = glmnet(x = x, y = asthma, family = "gaussian", alpha = alpha, lambda=NULL, standardize=FALSE)
print(paste0("GLMNETNOSTDRMSE:",sqrt(mean((predict(model, x)-asthma)^2))))
#
library(h2o)
h2o.init(nthreads=-1)
...
print(paste0("H2OSTDRMSE:",sqrt(mean((as.data.frame(h2o.predict(model, df.hex)[,1])-asthma)^2))))
...
print(paste0("H2ONOSTDRMSE:",sqrt(mean((as.data.frame(h2o.predict(model, df.hex)[,1])-asthma)^2))))
