...
model = pogsnet(x = x, y = asthma, family = "gaussian", alpha = alpha, lambda=NULL, intercept=intercept, cutoff=FALSE)
print(model)
print(paste0("RMSEPOGS for s:",sqrt(mean((predict(model, s=0.004938, newx=x)-asthma)^2))))
library(h2o)
library(glmnet)
model = glmnet(x = x, y = asthma, family = "gaussian", alpha = alpha, lambda=NULL, standardize=FALSE, intercept=intercept)
print(model)
print(paste0("GLMNET:",sqrt(mean((predict(model, s=0.004938, newx=x)-asthma)^2))))
#
#h2o.init(nthreads=-1)
