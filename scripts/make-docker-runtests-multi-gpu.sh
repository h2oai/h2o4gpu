...
# Don't use version in wheel name when find so local call to this script works without specific jenkins versions
# Just ensure clean dist/${platform}/*.whl before unstash in jenkins
$DOCKER_CLI exec ${CONTAINER_NAME} bash -c 'export NCCL_DEBUG=WARN && export HOME=`pwd` && cd repo && pip install `find /dot/src/interface_py/dist/'${platform}' -name "*h2o4gpu-*.whl"` -c /dot/src/interface_py/requirements_runtime_demos.txt && pip freeze && make '${target}
