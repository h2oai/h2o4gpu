...
# --build-arg http_proxy=http://172.16.2.142:3128/
$DOCKER_CLI build  -t opsh2oai/h2o4gpu-buildversion${extratag}-build -f Dockerfile-runtime-multi-gpu --rm=false --build-arg docker_name=${dockerimage} --build-arg use_miniconda=1 --build-arg python_version=${python_version} .
...
# Don't use version in wheel name when find so local call to this script works without specific jenkins versions
# Just ensure clean dist/${platform}/*.whl before unstash in jenkins
$DOCKER_CLI exec ${CONTAINER_NAME} bash -c 'export NCCL_DEBUG=WARN && export HOME=`pwd` && cd repo && pip install `find /dot/src/interface_py/dist/'${platform}' -name "*h2o4gpu-*.whl"` -c /dot/src/interface_py/requirements_runtime_demos_multi_gpu.txt && pip freeze && make '${target}
