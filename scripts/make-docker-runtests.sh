...
$DOCKER_CLI build  -t opsh2oai/h2o4gpu-buildversion${extratag}-build -f Dockerfile-runtime-dbg --rm=false --build-arg docker_name=${dockerimage} .
...
echo "Docker devel test and pylint - python -m pip install wheel from dist/${platform}, make ${target}"
...
$DOCKER_CLI exec ${CONTAINER_NAME} bash -c 'export HOME=`pwd` ; cd repo ; python -m pip install `find /dot/src/interface_py/dist/'${platform}' -name "*h2o4gpu-*.whl"`; python -m pip freeze ; make '${target}
