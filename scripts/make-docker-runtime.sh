...
echo "Building Runtime Docker on linux - Run"
...
echo "Building Runtime Docker on linux - pip freeze"
nvidia-docker exec ${CONTAINER_NAME} bash -c '. /h2o4gpu_env/bin/activate ; pip freeze'
echo "Building Runtime Docker on linux - Getting Data"
...
echo "Building Runtime Docker on linux - stopping docker"
nvidia-docker stop ${CONTAINER_NAME}
echo "Building Runtime Docker on linux - saving docker"
nvidia-docker save opsh2oai/h2o4gpu-${versionTag}${extratag}-runtime | gzip > h2o4gpu-${fullVersionTag}${extratag}-runtime.tar.gz
echo "Building Runtime Docker on linux Done"
