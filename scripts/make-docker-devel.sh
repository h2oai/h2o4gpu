...
$DOCKER_CLI exec ${CONTAINER_NAME} bash -c 'if [  $(git -C /dot/nccl rev-parse HEAD) !=  $(git -C /root/nccl rev-parse HEAD) ]; then echo "NCCL version mismatch in nccl submodule and docker file" && exit 1;  fi;'   
